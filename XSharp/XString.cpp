...
StringData::~StringData() { delete[] str; }

bool XChar::isDigit() const { return ucs >= '0' && ucs <= '9'; }

bool XChar::isSpace() const { return ucs == ' ' || ucs == '\t'; }
...
bool XChar::isLetterOrDigit() const { return isLetter() || isDigit(); }

char16_t XChar::value() const { return ucs; }

int XChar::digitValue() const { return ucs - '0'; }
...
bool XChar::operator==(XChar other) const { return ucs == other.ucs; }
...
XString::XString(const char* utf8) : XString(XString::fromUtf8(utf8)) {}
...
const XChar* XString::data() const { return (const XChar*)d.data()->str; }

XChar* XString::data() { return (XChar*)d.data()->str; }

int XString::size() const { return d.data()->size; }
...
XString& XString::append(const XString& other)
{
    ...
    if (d.ref->refCount() == 1) {
        ...
        xstrcpy(d.data()->str + d.data()->size, other.d.data()->str,
                otherSize + 1);
...
XString XString::operator+(XChar xc) const { return XString(*this).append(xc); }

XChar XString::operator[](int index) const { return d.data()->str[index]; }

XChar& XString::operator[](int index) { return data()[index]; }
...
void XString::removeAt(int pos)
{
    ...
    xstrcpy(d.data()->str + pos, d.data()->str + pos + 1,
            d.data()->size - pos - 1);
...
void XString::remove(int pos, int len)
{
    ...
    xstrcpy(d.data()->str + pos, d.data()->str + pos + len,
            d.data()->size - pos - len);
...
std::string XString::toStdString() const
{
    ...
    for (char16_t* p = (char16_t*)data(); p != (char16_t*)data() + size();
         ++p) {
        result.push_back(*p);
    }
    return result;
...
XString XString::fromStdString(std::string str) { return XString(str.data()); }

const XChar* XString::unicode() const { return data(); }

int XString::toInt(int base) const { return toInteger<int>(base); }

long long XString::toLongLong(int base) { return toInteger<long long>(base); }
...
XString XString::number(int v, int base) { return fromInterger<int>(v, base); }
...
XString::iterator XString::begin() { return (iterator)d.data()->str; }
...
bool XString::isDetach() { return d.ref->refCount() == 1; }
